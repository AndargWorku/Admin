...
const RolePage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('https://sheba-app.onrender.com/api/roles');
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`https://sheba-app.onrender.com/api/roles/${currentRole?.id}/delete`);
  ...
  const handleSave = async () => {
    try {
      if (currentRole) {
        await axios.put(`https://sheba-app.onrender.com/api/roles/${currentRole.id}/update`, formData);
      } else {
        await axios.post('https://sheba-app.onrender.com/api/roles/create', formData);
      }
      setShowModal(false);
      fetchData();
    } catch (error) {
      ...
      <table className="min-w-full bg-white border border-gray-300">
        <thead>
          <tr>
            ...
            <th className="py-2 px-4 border-b">Total Permissions</th>
        ...
        <tbody>
          {roles.map((role) => (
            <tr key={role.id}>
              ...
              <td className="py-2 px-4 border-b">{role.permissions.length}</td>
      ...
      {showModal && (
        <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-gray-800 bg-opacity-75">
          <div className="bg-white p-4 rounded shadow-lg">
            <h2 className="text-lg font-bold mb-4">{currentRole ? 'Edit Role' : 'Create Role'}</h2>
            <form>
              <div className="mb-4">
              ...
              <div className="mb-4">
                ...
                {/* Implement logic for displaying and selecting permissions */}
                <div>
