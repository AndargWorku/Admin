...
const RolePage: React.FC = () => {
  ...
  const handleDelete = async (role: Role) => {
    ...
    if (isConfirmed) {
      try {
        await axios.delete(`/api/roles/${role.id}/delete`);
        fetchData();
      } catch (error) {
  ...
  const handleSave = async () => {
    try {
      if (currentRole) {
        await axios.put(`/api/roles/${currentRole.id}/update`, formData);
      } else {
        await axios.post('/api/roles/create', formData);
      }
      setShowModal(false);
      fetchData();
    } catch (error) {
      ...
      {showModal && (
        <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-gray-800 bg-opacity-75">
          ...
          <div className="bg-white p-4 rounded shadow-lg">
            ...
            {/* Add permission selection here */}
            {/* Example: A list of checkboxes */}
            <div className="mb-4">
                ...
                <div key={role.id} className="flex items-center">
                  <input
                    ...
                    onChange={() =>
                      setFormData((prev) => ({
                        ...prev,
                        permissions: prev.permissions.includes(role.id)
                          ? prev.permissions.filter((id) => id !== role.id)
                          : [...prev.permissions, role.id],
                      }))
                    }
                  />
                  <label htmlFor={`permission-${role.id}`} className="ml-2">
                    {role.name}
                  </label>
                </div>
              ))}
